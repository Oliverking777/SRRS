...
export const DataProvider = ({ children }) => {
  ...
  const assets = {}; 
  const feature_list = []; 
  const illnessTypes = ["Respiratory Infection", "Gastrointestinal Illness", "Viral Infection", "Allergic Reaction", "Bacterial Infection", "Food Poisoning"];
    ...
    const unsubscribeReports = onSnapshot(collection(db, 'reports'), (snapshot) => {
      ...
      const recent = reportsList
        ...
        .map(report => ({
          ...
          status: report.severity === 'Severe' ? 'Critical' : 
                  report.severity === 'Moderate' ? 'Warning' : 'Normal',
          date: report.date,
        }));
    ...
    const fetchStaticData = async () => {
      try {
        ...
        trendsSnapshot.forEach(doc => {
          if (doc.id === 'report_trend_weekly') setReportTrendData(doc.data().data || []);
        ...
        regionsSnapshot.forEach(doc => {
          ...
          if (doc.id === 'top_regions') setTopRegionsData(doc.data().data || []);
          if (doc.id === 'admin_region_data') setAdminRegionData(doc.data().data || []);
        });
        ...
        illnessesSnapshot.forEach(doc => {
          ...
          if (doc.id === 'illness_data_admin') setIllnessDataAdmin(doc.data().data || []);
        });

        // Symptoms
        const symptomsSnapshot = await getDocs(collection(db, 'symptoms'));
        symptomsSnapshot.forEach(doc => {
          if (doc.id === 'symptoms_data') setSymptomsData(doc.data().data || []);
        });

        // Campaigns
        const campaignsSnapshot = await getDocs(collection(db, 'campaigns'));
        campaignsSnapshot.forEach(doc => {
          if (doc.id === 'campaigns_data') setCampaignsData(doc.data().data || []);
        });

        // Severity
        const severitySnapshot = await getDocs(collection(db, 'severity'));
        severitySnapshot.forEach(doc => {
          if (doc.id === 'severity_data') setSeverityData(doc.data().data || []);
        });
